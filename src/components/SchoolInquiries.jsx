...
import { FaTrash, FaCheck, FaEnvelope, FaWhatsapp, FaCalendarAlt, FaSearch, FaGraduationCap, FaBriefcase } from 'react-icons/fa';
import { motion, AnimatePresence } from 'framer-motion';

const SchoolInquiries = () => {
    const [schoolInquiries, setSchoolInquiries] = useState([]);
    const [mainInquiries, setMainInquiries] = useState([]);
    const [loading, setLoading] = useState(true);
    const [searchTerm, setSearchTerm] = useState('');
    const [filterStatus, setFilterStatus] = useState('all');
    const [activeTab, setActiveTab] = useState('academy'); // 'academy' or 'main'
    ...
    const filterInquiries = (inquiries) => {
    ...
    const currentInquiries = activeTab === 'academy' ? schoolInquiries : mainInquiries;
    const filteredInquiries = filterInquiries(currentInquiries);
        ...
        >
            <div className="card-header">
                <div className="sender-info">
                    ...
                    <div className="contact-links">
                        <a href={`mailto:${inquiry.email}`} title="Email">
                            <FaEnvelope /> {inquiry.email}
                        </a>
                        <a href={`https://wa.me/${inquiry.whatsapp}`} target="_blank" rel="noopener noreferrer" title="WhatsApp">
                            <FaWhatsapp /> {inquiry.whatsapp}
                        </a>
                    </div>
                </div>
                <div className="submission-meta">
                    <span className={`source-badge ${inquiry.source.toLowerCase().replace(' ', '-')}`}>
                        {inquiry.source}
                    </span>
                    <span>
                        <FaCalendarAlt /> {inquiry.submittedAt?.toLocaleString() || 'Recent'}
                    </span>
            ...
            <div className="card-content">
                <div className="content-group">
                    <label>{inquiry.source === 'Main Site' ? 'Interested Service' : 'Message'}</label>
        ...
        <div className="school-inquiries-container">
            ...
            <div className="inquiries-tabs">
                ...
                >
                    ...
                    <span className="tab-count">{schoolInquiries.length}</span>
                </button>
                <button
                ...
                >
                    <FaBriefcase />
                    <span>Main Site Inquiries</span>
                    <span className="tab-count">{mainInquiries.length}</span>
                </button>
            </div>
            ...
            {loading ? (
                ...
                <div className="empty-state">
                    ...
                    <p>
                        {activeTab === 'academy'
                            ? 'When students fill the academy contact form, they will appear here.'
                            : 'When customers fill the main site contact form, they will appear here.'}
                    </p>
                </div>
            ) : (
